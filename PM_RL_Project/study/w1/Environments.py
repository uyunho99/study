...
class HighwayGame(gym.Env):
    ...
    def get_encoded_obs(self) -> GymObs:
        ...
        :return:
        """
        return int(np.argmin(self.get_obs()))
    ...
    def step(self, action: int, debug=False) -> GymStepReturn:
        ...
        if debug:
            self.render()
            print(
                f"Action: {action}, Reward: {reward}, Done: {done}, Info: {info}")
    ...
    def play(self, policy=None):
        ...
        while not done:
            action = policy.action(
                obs) if policy else self.action_space.sample()
...
class HanoiGame(gym.Env):
    ...
    def __init__(self, n=3, n_towers=3):
        ...
        self.now = 0
        # this is only true for 3 towers, but we use this for maximum steps for all cases
        self.max_steps = 2**n - 1
    ...
    def reset(self):
        self.towers = [list(range(self.n, 0, -1))] + [[]
    ...
    def obs_to_str(self):
        render_str = ["\t|".join([str(t[i]) if i < len(
            t) else " " for t in self.towers]) for i in reversed(range(self.n))]
        return "\n".join(render_str)
    ...
    def check_action(self, action):
        ...
        invalid_action_conditions = [
            ...
            lambda frm_, to_: len(
                self.towers[to_]) > 0 and self.towers[frm_][-1] > self.towers[to_][-1]
        ]
